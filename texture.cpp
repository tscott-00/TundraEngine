...
Texture2DArray::Texture2DArray(const Descriptor& descriptor)
{
	/*if ((descriptor.image.width == 0 || descriptor.image.height == 0) && !descriptor.image.dataNeedsToBeLoaded)
	{
		texture = 0;
		isCounted = false;
		Console::Error("Texture " + descriptor.image.name + " has size of 0,0");
	}*/
...
{
	//std::cerr << name << " " << textureSize << " " << segmentPosX << " " << segmentPosY << " " << segmentSize << std::endl;
...
{
	//std::cerr << atlasData->atlasTexInfoMap.size() << std::endl;
